...
class User:
    ...
    def add_measurements(self,session_id,DF,FPS,bitrate,RTT):
        ...
        self.user_measurements[session_id]['RRT'].append(RTT)
...
class Stats_Calculator:
    ...
    def find_avg_DF(user):
        arr = np.array([])
        for sess in user.sessions:
            arr = np.concatenate((arr,np.array(user.user_measurements[sess.id]['DF'])))
        user.avg_DF = np.mean(arr)
        return user
    
    def find_avg_FPS(user):
        arr = np.array([])
        for sess in user.sessions:
            arr = np.concatenate((arr,np.array(user.user_measurements[sess.id]['FPS'])))
        user.avg_FPS = np.mean(arr)
        return user
    
    def find_avg_bitrate(user):
        arr = np.array([])
        for sess in user.sessions:
            arr = np.concatenate((arr,np.array(user.user_measurements[sess.id]['bitrate'])))
    ...
    def find_avg_bitrate(user):
        arr = np.array([])
        for sess in user.sessions:
            arr = np.concatenate((arr,np.array(user.user_measurements[sess.id]['RTT'])))
        user.avg_RTT = np.mean(arr)
        return user
    
    def estimate_next_session_time(user):
